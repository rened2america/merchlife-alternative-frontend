    ...
    <div className="mb-[250px]">
      ...
      <div className="p-5">
        <div className="flex flex-col lg:flex-row justify-center gap-4">
          <div className="flex flex-col w-full">
            ...
            <div className="flex flex-col w-full mt-4 overflow-x-auto">
              <table className="min-w-full">
                <thead className="border-b-8 border-b-white">
                  <tr className="bg-[#F3F3F3]">
                    ...
                    <th className="py-4 px-5 text-start">
                      <div className="border-r-2 border-r-gray">Products({products.length + 1})</div>
                    </th>
                    <th className="py-4 px-5 text-left">
                      <div className="border-r-2 border-r-gray">Price</div>
                    </th>
                    <th className="py-4 px-5 text-left">
                      <div className="border-r-2 border-r-gray">Quantity</div>
                    </th>
                    <th className="py-4 px-5 text-left">
                ...
                <tbody>
                  {product.map((product) => (
                    <tr key={product.id} className="bg-[#F3F3F3] border-b-white border-b-8">
                      ...
                      <td className="py-4 px-5">
                        <div className="flex items-center h-24 p-2 border-r-2 border-r-gray">
                          <img src={product.image} alt={product.name} className="w-24 h-24 object-cover border border-1 border-gray-400" />
                          <div className="ml-4">
                            <p>{product.name}</p>
                      ...
                      <td className="py-4 px-5">
                        <div className="py-8 px-6 h-24 flex items-center p-2 border-r-2 border-r-gray">${product?.price?.toFixed(2)}</div>
                      </td>
                      <td className="py-8 px-6">
                        <div className="py-8 px-6 h-24 flex items-center p-2 border-r-2 border-r-gray">
                          <select value={product.quantity} className="border rounded-full px-3 py-1 bg-[#F3F3F3] border-gray-400 focus:outline-none">
          ...
          <div className="w-full lg:w-1/3 p-8 h-72 bg-[#F3F3F3]">
            <div className="flex justify-between mb-4">
              <span className="text-lg font-semibold">Your Cart(2)</span>
              <span className="text-lg">$73.5</span>
            </div>
            <div className="flex flex-col items-end mb-4">
              <button className="bg-black h-14 text-white py-2 px-4 w-full mb-2 font-bold">CHECKOUT</button>
              <button className="border h-14 border-black py-2 px-4 w-full font-bold">UPDATE CART</button>
            </div>
